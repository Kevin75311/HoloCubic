...
#define WEATHER_NOW_API_UPDATE "https://v0.yiketianqi.com/api?unescape=1&version=v91&appid=%s&appsecret=%s&city=%s"
...
static void get_weather(void)
{
    ...
    if (httpCode > 0)
    {
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY)
        {
            ...
            run_data->wea.weather_code = weatherMap[sk["data"][0]["wea_img"].as<String>()];
            run_data->wea.temperature = sk["data"][0]["tem"].as<int>();

            // 获取湿度
            run_data->wea.humidity = 50;
            char humidity[8] = {0};
            strncpy(humidity, sk["data"][0]["humidity"].as<String>().c_str(), 8);
            humidity[strlen(humidity) - 1] = 0; // 去除尾部的 % 号
            run_data->wea.humidity = atoi(humidity);

            run_data->wea.maxTemp = sk["data"][0]["tem1"].as<int>();
            run_data->wea.minTemp = sk["data"][0]["tem2"].as<int>();
            // strcpy(run_data->wea.windDir, sk["data"][0]["win"].as<String>().c_str());
            // it is strange that the wind direction become cityname
            strcpy(run_data->wea.windDir, sk["city"].as<String>().c_str());
            run_data->wea.windLevel = windLevelAnalyse(sk["data"][0]["win_speed"].as<String>());
            run_data->wea.airQulity = airQulityLevel(sk["data"][0]["air"].as<int>());
        }
    }
    else
    {
